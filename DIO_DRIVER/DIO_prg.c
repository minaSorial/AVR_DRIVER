...
void DIO_vidInit(void){
	...
	for (u8PortNumLoc = 0; u8PortNumLoc < DIO_PORT_NUM; ++u8PortNumLoc) {
		...
		*(u8KarrDdra[pstrCurrentPortLoc->u8PortIdx])= pstrCurrentPortLoc->u8Direction & pstrCurrentPortLoc->u8Mask;
		...
		u8 u8PinNumLoc=0;
		for (u8PinNumLoc = 0; u8PinNumLoc < 8; ++u8PinNumLoc) {


			if ((BIT_GET( (  (pstrCurrentPortLoc->u8Direction) & (pstrCurrentPortLoc->u8Mask)),u8PinNumLoc)== DIO_OUTPUT)){

				*(u8KarrPorts[pstrCurrentPortLoc->u8PortIdx])= pstrCurrentPortLoc->u8InitValue & pstrCurrentPortLoc->u8Mask;
